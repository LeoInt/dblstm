...
void Net::Resize(int seqLength){
  ...
  cudaErrCheck(cudaMalloc((void**)&h_data_n,2* 2 * cell_dim_ * sizeof(float))); //ch
  cudaErrCheck(cudaMalloc((void**)&c_data_n,2* 2 * cell_dim_ * sizeof(float))); //ch
  ...
  cudaErrCheck(cudaMalloc((void**)&tmp_i_fw_n, 2*(seqLength+1) * 4 * cell_dim_ * sizeof(float))); //ch

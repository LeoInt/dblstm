...
static fp32 fp16_to_fp32_gpu_h(const fp16 in){
  unsigned int t1;
  unsigned int t2;
  unsigned int t3;
  fp32 out;
...
static void Print_matrix_to_file_16(const char nmfile[], fp16* mat, int n, int m, int r_c){
    //const char nmfile[] = "out.txt";
    std::ofstream outseis(nmfile); // output, normal file
    fp16 *data_host;
    data_host=(fp16*)malloc(n*m*sizeof(fp16));
    cudaMemcpy(data_host, mat, n*m*sizeof(fp16), cudaMemcpyDeviceToHost);  // this won't work, will throw error
    if(r_c==0){
      for (int jj=0; jj<n; jj++)
      {
        std::stringstream buf;
        ...
        for (ii=0; ii<m; ii++)
        {
          fp16* temp=(fp16 *)(data_host+jj*m+ii);
                    //printf("%f ", temp);
          buf<< fp16_to_fp32_gpu_h(*temp) <<" ";
                  //if(jj==101) printf("%f ", *temp);
        }
        outseis << buf.str() << "\n";
            //printf("\n%d %d row, col", jj, ii);
      ...
      for (int jj=0; jj<n; jj++)
      {
        ...
        for (ii=0; ii<m; ii++)
        {
          fp16* temp=(fp16 *)(data_host+ii*n+jj);
                    //printf("%f ", temp);
          buf<< fp16_to_fp32_gpu_h(*temp) <<" ";
                  //if(jj==101) printf("%f ", *temp);
        }
...
int main(int argc, char* argv[]) {
   ...
   N->Resize(seqLength); 
   	...
   	for(int i=0; i<numLayers; i++){
		...
		if(DEBUG){
   			...
   			Print_matrix_to_file_16(result1, L->bias_fw(), 4*hiddenSize, 1, 1);
   			sprintf(result2,"%s%d%s",s4,i,s2);
   			if(i==0)
   				Print_matrix_to_file_16(result2, L->wei_gifo_x_fw(), 4*hiddenSize, input_dim, 1);
   			else
   				Print_matrix_to_file_16(result2, L->wei_gifo_x_fw(), 4*hiddenSize, 2*hiddenSize, 1);
   			sprintf(result2,"%s%d%s",s5,i,s2);
   			Print_matrix_to_file_16(result2, L->wei_gifo_m_fw(), 4*hiddenSize, hiddenSize, 1); 
 			sprintf(result,"%s%d%s",pi,i,s2);
   			Print_matrix_to_file_16(result, L->phole_i_c_fw(), hiddenSize, 1, 1);
   			sprintf(result,"%s%d%s",pf,i,s2);
   			Print_matrix_to_file_16(result, L->phole_f_c_fw(), hiddenSize, 1, 1);
   			sprintf(result,"%s%d%s",po,i,s2);
   			Print_matrix_to_file_16(result, L->phole_o_c_fw(), hiddenSize, 1, 1);
  	
  			char s6[8]="bw.txt";
   			sprintf(result1,"%s%d%s",s3,i,s6);
   			Print_matrix_to_file_16(result1, L->bias_bw(), 4*hiddenSize, 1, 1);
   			sprintf(result2,"%s%d%s",s4,i,s6);
   			if(i==0)
   				Print_matrix_to_file_16(result2, L->wei_gifo_x_bw(), 4*hiddenSize, input_dim, 1);
   			else
   				Print_matrix_to_file_16(result2, L->wei_gifo_x_bw(), 4*hiddenSize, 2*hiddenSize, 1);
   			sprintf(result2,"%s%d%s",s5,i,s6);
   			Print_matrix_to_file_16(result2, L->wei_gifo_m_bw(), 4*hiddenSize, hiddenSize, 1); 
 			sprintf(result,"%s%d%s",pi,i,s6);
   			Print_matrix_to_file_16(result, L->phole_i_c_bw(), hiddenSize, 1, 1);
   			sprintf(result,"%s%d%s",pf,i,s6);
   			Print_matrix_to_file_16(result, L->phole_f_c_bw(), hiddenSize, 1, 1);
   			sprintf(result,"%s%d%s",po,i,s6);
   			Print_matrix_to_file_16(result, L->phole_o_c_bw(), hiddenSize, 1, 1);		
 		}
		N->AppendLayer(L);
   	}
  ...
  /*for(int i=0;i<seqLength;i++){
    for(int j=0;j<output_dim;j++){
      //float* temp=(float *)(data_host+jj*m+ii);
      float* aaa=(float *)(x_out + i*output_dim+j);
      printf("%f ", *aaa);
    }
    printf("\n");
  }*/

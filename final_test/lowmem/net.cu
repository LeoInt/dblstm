...
void Net::Resize(int seqLength){
  ...
  cudaErrCheck(cudaMalloc((void**)&tmp_i_fw_n, 4 * cell_dim_ * sizeof(float)));
  cudaErrCheck(cudaMalloc((void**)&tmp_h_bw_n, 4 * cell_dim_ * sizeof(float)));
  cudaErrCheck(cudaMalloc((void**)&tmp_i_bw_n, 4 * cell_dim_ * sizeof(float)));

  cudaMemset(h_data_n, 0, cell_dim_*sizeof(float));
  cudaMemset(c_data_n, 0, cell_dim_*sizeof(float));
  cudaMemset(h_data_bw_n + cell_dim_, 0, cell_dim_*sizeof(float));
  cudaMemset(c_data_bw_n + cell_dim_, 0, cell_dim_*sizeof(float));

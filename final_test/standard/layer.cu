...
return elapsedTime;  
  ...
  for (frame=0; frame < seqLength ; frame ++){
    ...
    elementWise_fp_1_w(gridDim, blockDim,stream ,
                      cell_dim_,
                       tmp_h_fw, 
                       tmp_i_fw + 4 * frame * cell_dim_, 
                       bias_fw_ ,
  ...
  elementWise_fp_2_w(gridDim, blockDim ,stream,
                      cell_dim_,
                       tmp_i_fw + 4 * frame * cell_dim_, 
                       c_data + (frame%2) * cell_dim_,
                       phole_o_c_fw_,
...
////backward//////////////////////////////////////////////////////////////////////
  ...
  for (frame=seqLength-1; frame >=0 ; frame --){
    ...
    gridDim.x = (4*cell_dim_ + blockDim.x - 1) / blockDim.x;               
               ...
               //element wise fuse to use
    elementWise_fp_1_w(gridDim, blockDim , stream, cell_dim_,
                       tmp_h_bw, 
                       tmp_i_bw + 4 * frame * cell_dim_, 
                       bias_bw_,
                       phole_i_c_bw_,
  ...
  elementWise_fp_2_w(gridDim, blockDim , stream, cell_dim_,
                       tmp_i_bw + 4 * frame * cell_dim_, 
                       c_data_bw + ((frame+1)%2) * cell_dim_,
                       phole_o_c_bw_,
                       h_data_bw + (frame%2) * cell_dim_,

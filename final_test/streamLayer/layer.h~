...
float Propagate(cublasHandle_t handle,  cudaStream_t stream_fw,  cudaStream_t stream_bw,float* in, float* out, int seqLength, float* tmp_h_fw, float* tmp_i_fw, float* tmp_h_bw, float* tmp_i_bw, float* h_data, float* c_data, float* h_data_bw, float* c_data_bw);  // Perform backward pass propagation, out_diff -> in_diff

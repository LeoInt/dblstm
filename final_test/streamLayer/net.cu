...
void Net::Resize(int seqLength){
  ...
  cudaErrCheck(cudaMalloc((void**)&h_data_n, (seqLength+1) * cell_dim_ * sizeof(float)));
  cudaErrCheck(cudaMalloc((void**)&c_data_n, (seqLength+1) * cell_dim_ * sizeof(float)));
  cudaErrCheck(cudaMalloc((void**)&c_data_bw_n, (seqLength+1) * cell_dim_ * sizeof(float)));
  cudaErrCheck(cudaMalloc((void**)&h_data_bw_n, (seqLength+1) * cell_dim_ * sizeof(float)));
  cudaErrCheck(cudaMalloc((void**)&tmp_h_fw_n, 4 * cell_dim_ * sizeof(float)));
  cudaErrCheck(cudaMalloc((void**)&tmp_i_fw_n, (seqLength+1) * 8 * cell_dim_ * sizeof(float)));
...
void Net::Feedforward(cublasHandle_t handle, cudaStream_t stream, float* in, float* out, int seqLength) {
  // we need at least 2 input buffers
  // propagate by using exactly 2 auxiliary buffers
  int L = 0;
  float time=0.f;
  
  time+=layers_[L]->Propagate(handle,stream, in, propagate_buf_ + (L%2)*seqLength * 2 * cell_dim_, seqLength, tmp_h_fw_n, tmp_i_fw_n, tmp_h_bw_n, tmp_i_bw_n, h_data_n, c_data_n, h_data_bw_n, c_data_bw_n);
//cudaDeviceSynchronize();
  for(L++; L<NumLayers(); L++) {
    time+=layers_[L]->Propagate(handle, stream, propagate_buf_ + ((L-1)%2)*seqLength*2*cell_dim_ ,propagate_buf_ + (L%2)*seqLength*2*cell_dim_, seqLength, tmp_h_fw_n, tmp_i_fw_n, tmp_h_bw_n, tmp_i_bw_n, h_data_n, c_data_n, h_data_bw_n, c_data_bw_n);
   //cudaDeviceSynchronize();  
  }
  time+=Af_l_->Propagate(handle, stream, propagate_buf_ + ((L-1)%2)*seqLength*2*cell_dim_, out, seqLength);

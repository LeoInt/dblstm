...
void Net::Feedforward(cublasHandle_t handle, float* in, float* out, int seqLength) {
  ...
  time+=layers_[L]->Propagate(handle,stream_fw, stream_bw, in, propagate_buf_ + (L%2)*seqLength * 2 * cell_dim_, seqLength, tmp_h_fw_n, tmp_i_fw_n, tmp_h_bw_n, tmp_i_bw_n, h_data_n, c_data_n, h_data_bw_n, c_data_bw_n);
cudaDeviceSynchronize();
  for(L++; L<NumLayers(); L++) {
    time+=layers_[L]->Propagate(handle, stream_fw, stream_bw, propagate_buf_ + ((L-1)%2)*seqLength*2*cell_dim_ ,propagate_buf_ + (L%2)*seqLength*2*cell_dim_, seqLength, tmp_h_fw_n, tmp_i_fw_n, tmp_h_bw_n, tmp_i_bw_n, h_data_n, c_data_n, h_data_bw_n, c_data_bw_n);
   cudaDeviceSynchronize();  
  }
